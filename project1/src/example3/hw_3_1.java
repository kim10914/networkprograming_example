// BorderLayout.SOUTH에 TextArea를 추가하여 TextField에 입력된 것이 파일이며 파일 내용을 디렉토리면 파일 목록이 출력
package example3;
...
public class hw_3_1 extends Frame implements ActionListener { // Frame 상속 받고 액션 리스너 구현
	private TextField enter; // 파일명 이나 디렉토리 경로를 입력받는 필드
	private TextArea output; // 결과를 다중행으로 보여주는 출력창
	//생성자
	public hw_3_1() { 
		super("File 클래스 테스트"); // Frame 생성자 호출
		enter = new TextField("파일 또는 디렉토리명을 입력하세요"); // 텍스트 필드 생성 문자열은 초기값
		...
		add(output, BorderLayout.SOUTH); // 컴포넌트 북쪽 배치
	...
	public void actionPerformed(ActionEvent e) { // 발생한 액션 이벤트 객체를 매개 변수로 받는 메서드
		...
		if(name.exists()) { // 파일 또는 디렉토리가 실제로 존재하는지 확인하는 메서드를 사용하여 검사
			...
			if(name.isFile()) { // 파일인지 검사
				...
				output.append("\n 디렉토리 내용\n"); // 화면 출력
				String[] list = name.list(); // 폴더 명을 받아 문자열 배열에 저장
				if(list != null) { // list 내용이 비어있지 않으면
					for (String item : list) { // 범위 기반 for 루프롤 이용
						output.append(item + "\n");
					}
				}
			}
